...
class Tooltip {
  ...
  _createTooltipTemplate() {
    ...
    const buttonRowWithSubtext = subtextMarkup === '' ? '' : 'chariot-btn-row-subtext';
    const template = `
      <div class="${this._classNames()}">
        ...
        <div class="chariot-btn-row ${buttonRowWithSubtext}">
          ${subtextMarkup}
          ${this._previousButtonMarkup()}
          <button class="btn btn-inverse btn-next">${this.ctaNext}</button>
        </div>
      </div>`;
  ...
  _previousButtonMarkup() {
    return this.tutorial.allowSteppingBackward && this.currentStepNum() > 1
      ? `<button class="btn btn-inverse btn-previous">${this.ctaPrevious}</button>`
      : '';
  }
